...
# desired input
img = cv2.imread('Input-Set/Cracked_02.jpg')
...
# bilateral filter(Intensity difference between the edges and neighbouring pixels: sigma colour sigma space border type)
bilateral = cv2.bilateralFilter(img_log, 5, 75, 75)

# Image segmentaion
# Canny Edge Detection(Single pixel width edges; treasholds)
edges = cv2.Canny(bilateral, 100, 200)

# Morphological Closing Operator(filling out shape)
kernel = np.ones((5, 5), np.uint8)  # (mask to cahnge sharpness, blurring )
